...
const ALIASES = new Map([
  ["hara/metaspec-metaspec", ["hal/metaspec"]]
]);
...
const slugify = (value) => value
  ...
  .replace(/^-+|-+$/g, "")
  .toLowerCase();
const titleFromPath = (filePath) => path.basename(filePath, path.extname(filePath)).replace(/[-_]+/g, " ").replace(/\b\w/g, (letter) => letter.toUpperCase());
...
const documentationPathFor = (sourcePath, manifestPaths) => {
  ...
  const candidates = [`${directory}/README.md`, `${path.posix.dirname(directory)}/README.md`, `${sourcePath.split("/")[0]}/README.md`];
...
export function selectCanonicalSpecifications(candidates) {
  ...
  for (const candidate of candidates) {
    const group = byId.get(candidate.id) || []; group.push(candidate); byId.set(candidate.id, group);
  }
  const selected = [];
  for (const group of byId.values()) {
    group.sort(candidateOrder);
    const [canonical, ...alternates] = group;
    selected.push({ ...canonical, ...(alternates.length ? { alternateSources: alternates.map(({ version, status, sourcePath }) => ({ version, status, path: sourcePath })) } : {}) });
  }
...
export function createRegistryIndex({ manifest, documents, repository = "hara-lang/hara-specs-registry", ref = "main", manifestPath = "spec-manifest.json", migration = null }) {
  const manifestPaths = new Set((manifest.files || []).map(({ path: filePath }) => filePath));
  const candidates = [];
  for (const file of manifest.files || []) {
    if (file.kind !== "edn" || file.path.startsWith("99-archive/") || file.path.includes("/conformance/") || !/(?:^|\/)(?:draft|ready|metaspec)(?:\/|$)/.test(file.path)) continue;
    const source = documents.get(file.path); if (!source) continue;
    const spec = parseSpecDocument(source, file, manifestPaths); if (spec) candidates.push(spec);
  }
  const specs = selectCanonicalSpecifications(candidates).map((spec) => ({ ...spec, source: { repository, ref, path: spec.sourcePath }, documentation: { repository, ref, path: spec.documentationPath } }));
  const layers = [...new Set(specs.map(({ layer }) => layer))].sort();
  const manifestJson = `${JSON.stringify(manifest, null, 2)}\n`;
  const catalogueJson = JSON.stringify(specs);
  return {
    schemaVersion: 2,
    source: { repository, ref, manifestPath, manifestDigest: digest(manifestJson), catalogueDigest: digest(catalogueJson), transitional: false, ...(migration ? { migration } : {}) },
    summary: { specifications: specs.length, draft: specs.filter(({ status }) => status === "draft").length, ready: specs.filter(({ status }) => status === "ready" || status === "stable").length, executable: specs.filter(({ executable }) => executable).length, requirements: specs.reduce((total, { requirements }) => total + requirements, 0), materialized: specs.length, pinnedSource: 0, layers },
    specs
  };
}
